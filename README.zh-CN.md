...
测试含真实原生编解码器、小模型 RGB 推理、SFT 中断恢复、三种后训练路径；另含待 Linux 验证的双进程 CPU DDP 测试；合成测试数据不代表论文性能。真实权重检查在三个数据集各一个样本上比较首个预测帧，80 个 token 及浮点像素均与原入口完全一致。详细边界见 `provenance/validation.json`。

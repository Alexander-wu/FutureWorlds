...
|---|---|---:|---|
| `models/main` | MemSPO (R0) | 500 | Paper main table |
...
128 trajectories, 32 predicted frames, 2 initial observations, per-frame Beam4, anchor1 + recent6, no true-future refresh. Recorded split: held-out development; runtime: GPU FP32.

| PSNR ↑ | SSIM ↑ | LPIPS ↓ |
|---:|---:|---:|
| 22.4699 | 0.802595 | 0.155104 |
...
Use `futureworlds.checkpoints.load_world_model(bundle_root, "rt1", variant="main")` for the predictor only. This does not decode RGB, prepare actions, or download T5 automatically. Use `futureworlds.pipeline.WorldPipeline` for the complete RGB/action/text pipeline; see `docs/LOADING.md`. Clean-environment full-horizon GPU equivalence remains pending. These models are custom action-conditioned predictors, not generic `pipeline("text-to-video")` models.

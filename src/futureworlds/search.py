"""Four disjoint beam groups, with unmodified policy scores for the RL surrogate.

Search penalties only affect proposal selection. Deterministic proposals do not
have on-policy sampling probabilities; the beam RL objective remains biased.
"""
...
def rollout_group_beam(
    ...
    # One input episode is expanded into groups * beams independent candidates.
    # context: [1, scene_tokens]; anchor: [1, frame_tokens]; actions: [T, A].
    model.eval()
    ...
    scores[::beams] = 0
    # Keep raw model scores separate from diversity-penalized search scores.
    ...
    for target in range(2, horizon + 2):
        ...
        for token in range(width):
            ...
            choices = []
            next_scores = []
            # Earlier groups provide same-position token counts for diversity.
            ...
            pending = chosen[:, None]
        # A surviving branch inherits its own entire history, not its old slot's.
        for f in frames:
    ...
    # Each group remains score-sorted; its first slot is the returned trajectory.
    selected = torch.arange(0, count, beams, device=device)
    frames = [
        {k: (v[selected] if torch.is_tensor(v) else v) for k, v in f.items()}
        for f in frames
    ]
    history = {t: x[selected] for t, x in history.items()}
    # Rebuild every scored prefix to catch branch/history mismatches.
    for f in frames:
        rebuilt, _ = make_prefix(
            context.expand(groups, -1), history, actions, f["target"]
        )
    ...
    return frames, dict(
        strategy="grouped_diverse_full_video_beam",
        groups=groups,
        beams_per_group=beams,
        active_paths=count,
        returned_trajectories=groups,
        penalty=penalty,

# Vendored from MIT-licensed RLVR-World/iVideoGPT, commit e1b8b6f40ca0696919ce9b8dbe965c4153bcf5f1.
"""
Ported from Paella
"""
...
class Discriminator3D(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(self, in_channels=3, cond_channels=0, hidden_channels=512, depth=6):
        super().__init__()
        d = max(depth - 3, 3)
        ...
        for i in range(depth - 1):
            ...
            layers.append(
                nn.utils.spectral_norm(
                    nn.Conv3d(c_in, c_out, kernel_size=3, stride=[1, 2, 2], padding=1)
                )
            )
            layers.append(nn.InstanceNorm3d(c_out))
            layers.append(nn.LeakyReLU(0.2))
        self.encoder = nn.Sequential(*layers)
        self.shuffle = nn.Conv3d(
            (hidden_channels + cond_channels) if cond_channels > 0 else hidden_channels,
            1,
            kernel_size=1,
        )
    ...
    def forward(self, x, cond=None):
        ...
        if cond is not None:
            cond = cond.view(
                ...
                1,
                1,
            ).expand(-1, -1, x.size(-3), x.size(-2), x.size(-1))
            x = torch.cat([x, cond], dim=1)
        x = self.shuffle(x)
        # x = self.logits(x)
        return x

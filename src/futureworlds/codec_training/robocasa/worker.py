"""DROID full-tokenizer transfer with distant targets and the prior paper GAN recipe."""
...
def main():
    ...
    train = rows(data / "train_episodes.jsonl")
    dev_all = rows(data / "dev_episodes.jsonl")
    tasks = sorted({r["task"] for r in dev_all})
    by_task = {
        t: sorted([r for r in dev_all if r["task"] == t], key=lambda r: r["id"])
        for t in tasks
    }
    dev = [
        by_task[t][i]
        for i in range(max(map(len, by_task.values())))
        for t in tasks
        if i < len(by_task[t])
    ]
    for r in train + dev:
        r["output"] = r["id"]
    assert len(train) > 3000 and len(dev) > 300
    assert not {r["output"] for r in train} & {r["output"] for r in dev}
    assert all(
        r["source_split"] == "target_human" and r["frames"] >= 2 for r in train + dev
    )
    if rank == 0:
        complete = json.loads((data / "complete.json").read_text())
        for part in ("train", "dev"):
            name = part + "_episodes.jsonl"
            assert (
                hashlib.sha256((data / name).read_bytes()).hexdigest()
                == complete["manifest_sha256"][name]
            )
        assert (
            hashlib.sha256(
                (
    ...
    if not args.smoke:
        assert json.loads((run / "gpu_preflight.json").read_text())["state"] == "PASSED"
    ...
    from .common import read_frames

    arr = read_frames(dev[rank], [0, 1, min(32, dev[rank]["frames"] - 1)])
    probe = (
    ...
    for shared in range(steps):
        ...
        for micro in range(accum):
            ...
            for j, (lo, hi) in enumerate(((1, 7), (8, 16), (17, 40))):
        ...
        if (
            ...
            if not args.smoke:
                ...
                save_due = step % cfg["checkpoint_every"] == 0 or step == steps
                if save_due:
    ...
    if args.smoke:
        ...
        assert (
            g_updates
            == cfg["discriminator_start"]
            + (cfg["steps"] - cfg["discriminator_start"]) // 2
            and d_updates == (cfg["steps"] - cfg["discriminator_start"]) // 2
        )
        log("FINAL_EVALUATION", final_shared_step=step, best=best)
        final_metrics = evaluation(model, dev[:512], details=True)
        if rank == 0:
            write(

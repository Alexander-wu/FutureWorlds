...
class CrossAttentionBlock(nn.Module):
    def __init__(
        ...
        self.att = nn.MultiheadAttention(
            channels, num_head, dropout=dropout, batch_first=True
        )
        self.resid_dropout = nn.Dropout(dropout)
        self.kv_norm = nn.GroupNorm(norm_group, channels)
        self.q_norm = nn.GroupNorm(norm_group, channels)
        self.kv_frames = kv_frames
        self.kv_pos_emb = nn.parameter.Parameter(
            torch.zeros((kv_frames * resolution * resolution // 4 * 5, channels)),
            requires_grad=True,
        )
        self.q_pos_emb = nn.parameter.Parameter(
            torch.zeros((resolution * resolution // 4 * 5, channels)),
            requires_grad=True,
        )
        self.act = get_activation(act_fn)

    def set_kv_frames(self, kv_frames):
        self.kv_pos_emb.data = self.kv_pos_emb.data[
            -kv_frames * self.kv_pos_emb.shape[0] // self.kv_frames :
        ]
        self.kv_frames = kv_frames
    ...
    def forward(self, z, addin):
        ...
        if self.kv_frames > 1:
            # B, t, C, H, W -> B, C, tH, W
            addin = addin.permute(0, 2, 1, 3, 4).reshape(
                addin.shape[0], addin.shape[2], -1, addin.shape[3]
            )
        kv = (
            self.kv_norm(addin)
            .permute(0, 2, 3, 1)
            .reshape(addin.shape[0], -1, addin.shape[1])
        )  # [B, HW, C]
        kv = kv + self.kv_pos_emb
        q = (
            self.q_norm(z).permute(0, 2, 3, 1).reshape(z.shape[0], -1, z.shape[1])
        )  # [B, HW, C]
        q = q + self.q_pos_emb
...
class ConditionalEncoder(Encoder):
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            if resolution <= max_att_resolution:
                self.cross_att_blocks.append(
                    CrossAttentionBlock(
                        output_channel, resolution, kv_frames=context_length
                    )
                )
        print("encoder num cross att blocks", len(self.cross_att_blocks))
    ...
    def forward(
        self, sample: torch.FloatTensor, cond_features: List[torch.FloatTensor]
    ) -> torch.FloatTensor:
        ...
        for i, down_block in enumerate(self.down_blocks):
            ...
            if sample.shape[-2] <= self.max_att_resolution:
...
class ConditionalDecoder(Decoder):
    def __init__(
        ...
        print("decoder num cross att blocks", len(self.cross_att_blocks))

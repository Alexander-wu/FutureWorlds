...
class CompressiveVQModelFSQ(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.quant_linear = nn.Linear(
            self.dyna_latent_channels * self.patch_size * self.patch_size,
            dyn_vq_embed_dim,
        )
        self.dynamics_quantize = FSQ(
            levels=dyn_fsq_levels,
        )
        self.post_quant_linear = nn.Linear(
            dyn_vq_embed_dim,
            self.dyna_latent_channels * self.patch_size * self.patch_size,
        )
    ...
    @apply_forward_hook
    def encode(
        self, encoder, x: torch.FloatTensor, return_dict: bool = True
    ) -> CompressiveVQEncoderOutput:
        h, d = encoder(x)
        h = self.quant_conv(h)
    ...
    def decode(
        ...
        if self.context_length > 1:
            ...
            cond_features = [
                f.reshape(B, self.context_length, *f.shape[-3:])
                .unsqueeze(1)
                .repeat(1, self.segment_len, 1, 1, 1, 1)
                .reshape(-1, self.context_length, *f.shape[-3:])
                for f in cond_features
            ]  # B*(T-t), t, C, H, W
        else:
            cond_features = [
                f.unsqueeze(1)
                .repeat(1, self.segment_len, 1, 1, 1)
                .reshape(-1, *f.shape[-3:])
                for f in cond_features
            ]
        ...
        return CompressiveVQDecoderOutput(
            sample=dec,
            ref_sample=ref_dec,
            commit_loss=commit_loss,
            dyn_commit_loss=dyn_commit_loss,
        )
    ...
    def tokenize(self, pixel_values: torch.FloatTensor, context_length: int = 1):
        ...
        if self.context_length > 1:
            ...
            cond_features = [
                f.reshape(B, self.context_length, *f.shape[-3:])
                .unsqueeze(1)
                .repeat(1, future_length, 1, 1, 1, 1)
                .reshape(-1, self.context_length, *f.shape[-3:])
                for f in cond_features
            ]  # B*(T-t), t, C, H, W
        else:
    ...
    def detokenize(
        ...
        quant = quant.reshape(
            B * context_length, ctx_res[0], ctx_res[1], len(self.vq_fsq_levels)
        ).permute(0, 3, 1, 2)  # [B, D, H, W]
        quant2 = self.post_quant_conv(quant)

        quant_d = self.dynamics_quantize.indices_to_codes(indices_d)
        quant_d = quant_d.reshape(
            -1, dyn_res[0] * dyn_res[1], len(self.dyn_fsq_levels)
        )  # [B, L, D]
        quant2_d = self.post_quant_linear(quant_d)
        ...
        if context_length > 1:
            ...
            cond_features = [
                f.reshape(B, context_length, *f.shape[-3:])
                .unsqueeze(1)
                .repeat(1, future_length, 1, 1, 1, 1)
